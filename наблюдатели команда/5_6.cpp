...
public:
    ...
    void execute() override
    {
        sf::Shape& shape = mPool.getShape(mIndex);
        mOldColor = shape.getFillColor();
        mNewColor = getRandomColor();
        shape.setFillColor(mNewColor);
    }
...
protected:
    ShapePool& mPool;
    std::vector<sf::Vector2f> mOldPositions;
...
public:
    ...
    void execute() override
    {
        ...
        for (size_t i = 0; i < mPool.getNumbeorOfShapes(); ++i)
        {
            ...
            mOldPositions.push_back(shape.getPosition());

            sf::Vector2f min = shape.getGlobalBounds().getSize() / 2.0f;
            sf::Vector2f max = mPool.getSize() - shape.getGlobalBounds().getSize() / 2.0f;
            shape.setPosition(mPool.getPosition() + sf::Vector2f(getRandomFloat(min.x, max.x), getRandomFloat(min.y, max.y)));
        }
    }

    void unexecute() override
    {
        for (size_t i = 0; i < mOldPositions.size(); ++i)
        {
    ...
    void handleEvent(sf::Event event)
    {
        for (std::size_t i = 0; i < mButtons.size(); ++i)
        {
            ...
            if (result)
            {
                ...
                mHistory.push_back(mCommands.at(i));
            }
        }
    }
...
int main()
{
    ...
    control.addButton(
    new Button(window, {40, 500, 240, 40}, font, "Colors + Positions"),
        new MultiCommand({
    ...
    control.addButton(new Button(window, {40, 440, 240, 40}, font, "Clear All Shapes"), new ClearAllShapesCommand(pool));
    control.addButton(new Button(window, {40, 380, 240, 40}, font, "All Random Colors"), new RandomAllColorsCommand(pool));
    control.addButton(new Button(window, {40, 700, 240, 40}, font, "Undo"), new NoCommand);
    ...
    while (window.isOpen())
    {
        ...
        while (window.pollEvent(event))
        {
            ...
            {
                if (control.getNumberOfButtons() > 8 && control.getButton(8)->isHovered())
                    control.undoLast();
                else
